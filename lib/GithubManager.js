...
var TOKEN = false;
var access_note = 'auth for gitbook-setup on ' + os.hostname() + " " + os.type() + " " + os.platform() + " at " + Date();
...
/*
  ...
  */
      ...
      if (TOKEN) {
        askForRemoteRepo(function (answers) {
          if (answers.create_repo == 'No') {
            ...
            if (answers.repo_name.includes("/")) { // Create repo in organization
              var split_str = answers.repo_name.split("/");
              console.log("Organization is: " + split_str[0])
              console.log("Repo name is: " + split_str[1]);
    ...
    // this method checks if the authenticated user is administrator of the organization given by argument.
    // ARGS: org -> the name of the organization.
    // CALLBACK: returns (isAdmin)
    checkAdminOrg (org, callback) {
      var owner_usernames;
      var is_owner = false;
      ...
      github.orgs.getMembers({
        ...
        owner_usernames = out.data;
        github.users.get({}, (err, out) => {
          owner_usernames.forEach ((owner, inx) => {
            if (out.data.login == owner.login) {
              callback(true);
              is_owner = true;
            }
            else if (inx + 1 == owner_usernames.length && !is_owner) {
              callback(false);
            }
          })
        })
