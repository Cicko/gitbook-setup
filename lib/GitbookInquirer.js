  ...
  static ask (callback) {
    this.questions = [
      ...
      },
      {
        ...
        choices: ['gh-pages', 'heroku', 'server\\s'],
      },
      {
        type: 'input',
        name: 'deployServers',
        message: 'Indicate your deploy server name (url or ip). You can indicate more than one separating by commas: ',
        when: function (answers) {
          return answers.deploys.includes('server\\s');
        },
        filter: function (val) {
          return val.split(',');
        }
    ...
    inquirer.prompt(this.questions).then(function (answers) {
      if (answers.deploys.includes("server\\s")) {
        var i = answers.deploys.indexOf("server\\s");
        if(i != -1) {
